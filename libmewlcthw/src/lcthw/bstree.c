...
{
  if(map) {
    BSTree_traverse(map, BSTreeNode_destroy_cb);
    free(map);
  }
}
...
// 
static inline void BSTree_replace_node_in_aspect_of_parent(BSTree *map, BSTreeNode *node, BSTreeNode *new_node)
{
  if(node->parent) {
...
static inline void BSTreeNode_swap_data(BSTreeNode *a, BSTreeNode *b)
{
  void *temp = NULL;
...
static inline BSTreeNode *BSTree_node_delete(BSTree *map, BSTreeNode *node, void *key)
{
  ...
  if(cmp < 0) {
    ...
    if(node->left && node->right) {
      ...
      BSTreeNode_swap_data(successor, node);
      // this leaves the old successor with possibly a right child
      // so replace it with that right child
      //
      BSTree_replace_node_in_aspect_of_parent(map, successor, successor->right);
      // finally it's swapped, so return successor insteadof node
      return successor;
    } else if(node->left) {
      BSTree_replace_node_in_aspect_of_parent(map, node, node->left);
    } else if(node->right) {
      BSTree_replace_node_in_aspect_of_parent(map, node, node->right);
    } else {
      BSTree_replace_node_in_aspect_of_parent(map, node, NULL);
    }
    return node;
  }
}
